...
public class PicEncryption {
  ...
  public Pane ImagePane() {
    ...
    JFXTooltip tooltip1 = new JFXTooltip(
        "操作提示：\n将图片拖入左侧灰色框内\n在文本框内输入加密参数\n注意，加密参数必须是0-1之间的小数\n否则无法加密\n在应用根目录获取加密好的图片\n输入加密时的加密参数以解密");
    ...
    confirmButton1.setOnAction(event -> {
      ...
      try {
        ...
        iv.setImage(new Image(new FileInputStream("result"+new File(Constants.IMAGENAME).getName())));
    ...
    form.getChildren()
        .addAll(tipBtn, fileTip, filesBox1, codeTip, code, confirmButton1, separator, fileDecTip,
            filesBox2, codeDecText, decode, confirmButton2);
